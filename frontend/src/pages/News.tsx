...
const News = () => {
  ...
  useEffect(() => {
    ...
    let query = `/articles?category=News&take=${limit}&skip=${skip}`;
  ...
  const featuredPost = !isFiltering && news.length > 0 ? news[0] : null;
  const recentNews = !isFiltering && news.length > 1 ? news.slice(1) : news;
      ...
      <div className="container">
        ...
        <div className={styles.hero}>
          <div className={styles.heroContent}>
            ...
            <p className={styles.subtitle}>Latest updates, funding rounds, and startup acquisitions.</p>
          ...
          <div className={styles.heroImageWrapper}>
            <img 
              src="/images/news_main_cityscape.jpg" 
              alt="Industry News" 
        ...
        <div className={styles.mainLayout}>
          <div className={styles.feed}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 'var(--spacing-4)', marginBottom: 'var(--spacing-2)' }}>
              <h3 style={{ margin: 0 }}>{selectedTag ? `News matching "${selectedTag}"` : 'Recent Updates'}</h3>
              {selectedTag && (
                <button 
              ...
              {recentNews.length > 0 ? (
                ...
                !isLoading && featuredPost === null && (
                  <div style={{ textAlign: 'center', padding: '3rem', color: 'var(--text-secondary)' }}>
                    No news available matching your criteria.
                  </div>
                )
              )}
              {isLoading && news.length > 0 && (
            ...
            <div className={styles.sidebarSection}>
              ...
              <div className={styles.authorList}>
                {[
                  ...
                  <div key={idx} className={styles.authorItem} style={{ alignItems: 'center', padding: '0.5rem 0' }}>
                    <div className={styles.avatarSmall} style={{ width: '48px', height: '48px', fontSize: '1.25rem' }}>{author.initials}</div>
                    <div style={{ flex: 1, marginLeft: '8px' }}>
                      <div style={{ fontWeight: 600, fontSize: '1.125rem' }}>{author.name}</div>
                      <div style={{ fontSize: '0.95rem', color: 'var(--text-secondary)' }}>{author.role}</div>
                    </div>
                  </div>
                ))}
              </div>
