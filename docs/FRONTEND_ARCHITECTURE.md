...
Dự án Frontend được setup bằng React 19, TypeScript, và Vite. ## 1. Cấu trúc thư mục (Folder Structure)
...
### 2.1 CSS Variables
Tất cả mã màu, spacing, font-size phải được lấy từ file `src/styles/variables.css`. Ví dụ: `color: var(--primary-500);` thay vì dùng mã HEX hardcode.
